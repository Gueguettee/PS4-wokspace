...
void uart3RXInterrupt( void )
{
    ...
    switch(uartChar)
    {
        ...
        case CHAR_VERIN_ON:
            xbeeWriteChar(CHAR_VERIN_ON);
            break;
            
        case CHAR_VERIN_OFF:
            xbeeWriteChar(CHAR_VERIN_OFF);
            break;
            
        case CHAR_BIG_WHEEL_ON:
            xbeeWriteChar(CHAR_BIG_WHEEL_ON);
            break;
            
        case CHAR_BIG_WHEEL_OFF:
            xbeeWriteChar(CHAR_BIG_WHEEL_OFF);
            break;
        ...
        switch(state)
        {
            ...
            case RUN:
            {
                ...
                if(firstLoop)
                {
                    ...
                    setPwmDuty(ePWM3, SERVO_CLOPET_DUTY_CLOSE);
                    pwmEnable(ePWM2);
                    pwmEnable(ePWM3);
                        ...
                        ((tempSpeed[eJoyY]!=0)&&(tempSpeed[eJoyX]!=lastSpeed[eJoyX])))
                {
                    ...
                    else
                    {
                        if(tempSpeed[eJoyX] > 0)
                        {
                            ...
                            {
                                setPwmDuty(ePWM1, 
                                    ((10000/N_STEP_JOY*(uint16_t)tempSpeed[eJoyX])
                ...
                if(fClapet == true)
                {
                    ...
                    {
                        setPwmDuty(ePWM3, SERVO_CLOPET_DUTY_OPEN);
                        stateClapet = true;
                    }
                    fClapet = false;
                }
