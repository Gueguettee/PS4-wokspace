...
#ifdef ML180V3
static Callback_t cbf_uart1_rx_pointer = 0;
static Callback_t cbf_uart1_tx_pointer = 0;
#endif
...
void uartInit(uart_t eUARTx, uint32_t baud){
    ...
    switch(eUARTx)
    {
#ifdef ML180V3
        case eUART1:
            TRISGbits.TRISG0 = 1;   // (re)Set U1RX as input
            ...
            //*************************************************************
            RPOR10bits.RP113R  = 0b000001;   // Assign U1TX To Pin RP113
            //*************************************************************
            // Configure Input Functions RPIn Pins
            // (See TABLE 11-1 & TABLE 4-33)
            //*************************************************************
            RPINR18bits.U1RXR  = 0b1110000;  // Assign U1RX To Pin RPI112
            //*************************************************************
            // Lock Registers
            //*************************************************************
            __builtin_write_OSCCONL(OSCCON | (1 << 6));

            U1MODEbits.UARTEN = 0;	// TX, RX DISABLED, ENABLE at end of function
            ...
            U1MODEbits.BRGH = BRGH_setting; // High Baud Rate Enable bit

            U1BRG = BRG_val;        // UART Baud Rate setting
            ...
            IPC3bits.U1TXIP = 1;    // Set IRQ priority level
            IFS0bits.U1TXIF = 0;    // Clear the Transmit Interrupt Flag
            IEC0bits.U1TXIE = 1;    // Enable Transmit Interrupts

            IPC2bits.U1RXIP = 1;    // Set IRQ priority level
		...
		case eUART2:
            ...
            U2MODEbits.UARTEN = 0;	// TX, RX DISABLED, ENABLE at end of function
		...
		case eUART3:
            ...
            //*************************************************************
#ifdef ML180V3
            RPOR12bits.RP127R = 0b011011;   // Assign U3TX To Pin RP127
#else
#ifdef ML180V2
            ...
            //*************************************************************
#ifdef ML180V3
            //RPINR27bits.U3RXR  = 0b1111001;  // Assign U3RX To Pin RPI121
            RPINR27bits.U3RXR  = 0b0011011;  // Assign U3RX To Pin RPI121
#else
#ifdef ML180V2
            ...
            U3MODEbits.UARTEN = 0;	// TX, RX DISABLED, ENABLE at end of function
...
#ifdef ML180V3
        case eUART1:
			while(1 == U1STAbits.UTXBF);    // Wait if Transmit buffer is full
...
#ifdef ML180V3
        case eUART1:
			while (0 == U1STAbits.URXDA);   // Wait for data to be available (when polling)
			return U1RXREG;
			break;
#endif
		case eUART2:
...
void registerUARTxCallback(uart_t eUARTx, rx_tx_t rxtx, Callback_t cbf_pointer)
{
    switch (eUARTx)
    {
        ...
        case eUART1:
            if (eRX==rxtx)
            {
                cbf_uart1_rx_pointer = cbf_pointer;
            }
            else
            {
                cbf_uart1_tx_pointer = cbf_pointer;
            }
            break;
#endif
        case eUART2:
...
void __attribute__((__interrupt__, __auto_psv__)) _U1RXInterrupt(void)
{
  ...
  if (0!=cbf_uart1_rx_pointer)
  {
    cbf_uart1_rx_pointer();
  }
}
...
{
  IFS0bits.U1TXIF = 0;    // Clear the Transmit Interrupt Flag
  if (0!=cbf_uart1_tx_pointer)
  {
    cbf_uart1_tx_pointer();
  }
}
#endif
// UART2 RX interrupt
