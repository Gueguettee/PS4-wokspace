...
/// @param   ePORTx Port on which you want to read from
/// @return  The whole PORT bits read value
/// @note    Allow to read an entire port
...
/// @param   ePORTx Port on which you want to write to
/// @param   value Value that the port is going to have
/// @return  true if successful, false if failed
/// @note    This function allow to write an entire Port (be carreful that this
///          function overwrite the entire LATx register)
bool gpioPortWrite(port_t ePORTx, uint16_t value);

/// @brief   Write to the selected GPIO PORT bit(s)
/// @param   ePORTx Port on which you want to write to
/// @param   BIT Bit(s) that you want to write to
...
/// @return  true if successful, false if failed
/// @note    This function allow to configure an entire Port (be carreful that
///          this function overwrite the entire TRISx register)
bool gpioPortConfig(port_t ePORTx, uint16_t value);

/// @brief   Configure the selected GPIO PORT bit(s) direction
/// @param   ePORTx Port you want to configure direction
...
///          multi-bits config: gpioConfig(ePORTA, (pinRA11|pinRA12|pinRA14), INPUT);
bool gpioBitConfig(port_t ePORTx, uint16_t BIT, uint16_t direction);

/// @brief   Configuration of GPIO External interrupt
/// @param   eINTx INTx that you wont to activate
/// @param   edgeSelect Let you choose if INTx occurs on the rising or falling edge
/// @note    This function allow to enable any INTx. For INT1 and INT2 there is
///          a remapping to the pin pinRA11 and pinRA12
...
/// @param   intx intx that you want to register
/// @param   cbf_pointer Address of the function to register
/// @note    This function allow to register all external interrupt routines by 
///          a callback.
