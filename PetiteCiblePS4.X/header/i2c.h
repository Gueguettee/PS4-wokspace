...
/// @param   SCLx frequency in kHz
/// @param   Select 7/10 bits slave address
/// @param   ePullUpxx [optional] Internal pull-up ON/OFF
/// @note    By default, internal pull-up are not connected. If they are connected
///          (no external pull-up on I2C device), I2C module may work only at low speed (<100 kbit/s)
///          - Valid function call examples:
///            void i2cInit(i2c_t eI2Cx, uint16_t fscl, i2cAddrLength_t slave_addr_length);
///            void i2cInit(i2c_t eI2Cx, uint16_t fscl, i2cAddrLength_t slave_addr_length, pullup_on_off_t ePullxx);
...
/// @brief   This function provides status back to user if I2CRCV register
///          contain data.
/// @param   eI2Cx - I2C you want to use to receive a data byte
/// @note    This function determines if there is any byte to read from I2CRCV register
...
/// @note    This function allow to register the I2C interrupt routines by 
///          a callback.
