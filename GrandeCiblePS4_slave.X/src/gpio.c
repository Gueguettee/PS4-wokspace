...
bool gpioBitWrite(port_t ePORTx, uint16_t BIT, uint16_t value)
{
	...
	switch(ePORTx)
    {
		...
		case ePORTF:
			...
			if(HIGH == value)
            {
				LATF = (oldLatValue | BIT); // Masking the old value, so the desired bit(s) are set
			}
			else
            {
				LATF = (oldLatValue & invertedBitValue); // Masking the old value, so the desired bit(s) are cleared
			}
			break;
		case ePORTG:
			oldLatValue = LATG;
...
bool gpioBitConfig(port_t ePORTx, uint16_t BIT, uint16_t direction)
{
	...
	switch(ePORTx)
    {
		...
		case ePORTF:
			...
			if(INPUT == direction)
            {
				TRISF = (oldTrisValue | BIT); // Masking the old value, so the desired bit(s) are set as input
			}
			else
            {
				TRISF = (oldTrisValue & invertedBitValue); // Masking the old value, so the desired bit(s) are set as output
			}
			break;
		case ePORTG:
			oldTrisValue = TRISG;
...
void externInterruptInit(extINT_t eINTx, edge_t edgeSelect)
{
    ...
    {
		case eINT0:
            TRISBbits.TRISB7 = 0x1;     // (re)Set RB7/INT0 as input
            
            INTCON2bits.INT0EP = edgeSelect;    // Set INTx edge
            
            // Interrupt Controller Settings
            IPC0bits.INT0IP = 4;        // Set INT0 IRQ priority level
			IFS0bits.INT0IF = 0;        // Reset INT0 interrupt flag
			IEC0bits.INT0IE = 1;        // Enable INT0 Interrupt Service Routine
			break;
		case eINT1:
            //TRISBbits.TRISB14 = 0x1;    // (re)Set PWM1H/INT1 as input
			TRISAbits.TRISA11 = 0x1;    // (re)Set RA11/INT1 as input
            ...
            RPINR0bits.INT1R    = 0x1B; // Assign INT1 To Pin RPI127
			//*************************************************************
			// Lock Registers
			//*************************************************************
			__builtin_write_OSCCONL(OSCCON | (1 << 6));
